...
const filterTransactions = (walletAddress) => (tx) => {
    const topics = tx.topics.map(topic => parseInt(topic));
    const me = parseInt(walletAddress);
    return topics.indexOf(me) > -1;
}
    ...
    axios.get(`https://api.etherscan.io/api?module=logs&action=getLogs&fromBlock=0&toBlock=latest&address=${CONTRACT_ADDRESS}&topic0=${OPERATION_TRANSFER_HASH}&topic1=${walletAddress}&topic0_1_opr=or&topic2=${walletAddress}&topic0_2_opr=or`)
        .then(({ status, data }) => {
            if (status === 200 && data.status === "1") {
                const payload = data.result.filter(filterTransactions(walletAddress));
